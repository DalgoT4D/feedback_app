...
if active_cycle:
    ...
    with col1:
        st.info(f"**{active_cycle['cycle_display_name'] or active_cycle['cycle_name']}**")
    ...
    with col2:
        ...
        if active_cycle['cycle_year'] and active_cycle['cycle_quarter']:
            st.write(f"Period: {active_cycle['cycle_year']} {active_cycle['cycle_quarter']}")
else:
    st.warning("No active review cycle found. Create a new review cycle to begin the feedback process.")
...
if active_cycle:
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("Modify Cycle Deadlines"):
            st.session_state.show_modify_form = True
    with col2:
        if st.button("Mark Cycle Complete"):
            st.session_state.show_complete_form = True
    
    st.info("**Note:** You cannot create a new cycle while one is active. Modify the current cycle or mark it complete first.")
    ...
    with st.form("new_cycle_form"):
        ...
        with col2:
            cycle_year = st.selectbox(
                "Year",
                options=[datetime.now().year, datetime.now().year + 1],
                index=0
            )
        ...
        if create_cycle:
            if display_name and description:
                ...
                try:
                    ...
                    approval_deadline_str = approval_deadline.strftime('%Y-%m-%d')
                    feedback_deadline_str = feedback_deadline.strftime('%Y-%m-%d')
                    results_deadline_str = results_deadline.strftime('%Y-%m-%d')
                    
                    result = create_named_cycle(
                        display_name, description, cycle_year, cycle_quarter,
                        internal_name, nomination_start_str, nomination_deadline_str, 
                        approval_deadline_str, feedback_deadline_str, results_deadline_str, user_id
                    )
...
if st.session_state.get('show_modify_form', False):
    st.subheader("Modify Cycle Deadlines")
    
    with st.form("modify_cycle_form"):
        st.info(f"Modifying deadlines for: **{active_cycle['cycle_display_name'] or active_cycle['cycle_name']}**")
        
        # Convert current dates for display
        from datetime import datetime
        current_nomination_deadline = datetime.strptime(active_cycle['nomination_deadline'], '%Y-%m-%d').date()
        current_approval_deadline = datetime.strptime(active_cycle['approval_deadline'], '%Y-%m-%d').date()
        current_feedback_deadline = datetime.strptime(active_cycle['feedback_deadline'], '%Y-%m-%d').date()
        current_results_deadline = datetime.strptime(active_cycle['results_deadline'], '%Y-%m-%d').date()
        
        col1, col2 = st.columns(2)
        with col1:
            new_nomination_deadline = st.date_input(
                "Nomination Deadline", 
                value=current_nomination_deadline
            )
            new_approval_deadline = st.date_input(
                "Approval Deadline", 
                value=current_approval_deadline
            )
        ...
        with col2:
            ...
            new_results_deadline = st.date_input(
                "Results Deadline", 
                value=current_results_deadline
            )
        
        # Submit buttons
        col1, col2 = st.columns(2)
        with col1:
            update_cycle = st.form_submit_button("Update Deadlines", type="primary")
        with col2:
            cancel_modify = st.form_submit_button("Cancel")
        
        if update_cycle:
            # Convert dates to strings
            nomination_deadline_str = new_nomination_deadline.strftime('%Y-%m-%d')
            approval_deadline_str = new_approval_deadline.strftime('%Y-%m-%d')
            feedback_deadline_str = new_feedback_deadline.strftime('%Y-%m-%d')
            results_deadline_str = new_results_deadline.strftime('%Y-%m-%d')
            
            try:
                if update_cycle_deadlines(
                    active_cycle['cycle_id'], nomination_deadline_str, approval_deadline_str,
                    feedback_deadline_str, results_deadline_str
                ):
                    st.success("Cycle deadlines updated successfully!")
                    st.session_state.show_modify_form = False
                    st.rerun()
                else:
                    st.error("Error updating cycle deadlines")
            except Exception as e:
                st.error(f"Exception updating deadlines: {str(e)}")
...
if st.session_state.get('show_complete_form', False):
    ...
    with st.form("complete_cycle_form"):
        st.warning(f"You are about to mark **{active_cycle['cycle_display_name'] or active_cycle['cycle_name']}** as complete.")
        ...
        if complete_cycle:
            try:
                success, message = mark_cycle_complete(active_cycle['cycle_id'], completion_notes)
                if success:
...
if active_cycle: # Wrap the entire section
    ...
    if current_phase == "Nomination Phase":
        ...
        from datetime import datetime
        nomination_deadline = datetime.strptime(active_cycle['nomination_deadline'], '%Y-%m-%d').date()
        days_left = (nomination_deadline - datetime.now().date()).days
        
        if days_left > 0:
            st.write(f"**{days_left} days** left for nominations")
        else:
            st.warning("**Nomination deadline has passed!** Consider extending or moving to approval phase.")
        ...
        from datetime import datetime
        approval_deadline = datetime.strptime(active_cycle['approval_deadline'], '%Y-%m-%d').date()
        days_left = (approval_deadline - datetime.now().date()).days
        
        if days_left > 0:
            st.write(f"**{days_left} days** left for approvals")
        else:
            st.warning("**Approval deadline has passed!** Follow up with managers urgently.")
        ...
        from datetime import datetime
        feedback_deadline = datetime.strptime(active_cycle['feedback_deadline'], '%Y-%m-%d').date()
        ...
        from datetime import datetime
        results_deadline = datetime.strptime(active_cycle['results_deadline'], '%Y-%m-%d').date()
...
if active_cycle: # Wrap the entire section
    ...
    if all_cycles:
        if len(all_cycles) > 1 or (len(all_cycles) == 1 and not all_cycles[0]['is_active']):
