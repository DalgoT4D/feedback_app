...
from services.db_helper import (
    get_active_review_cycle, get_user_deadline, 
    get_feedback_progress_for_user, is_deadline_passed,
    get_pending_reviews_for_user, get_pending_reviewer_requests
)
...
if not active_cycle:
    ...
    st.info("Please check back later or contact HR for information about upcoming review cycles.")
    st.stop()
...
with col2:
    feedback_deadline_date = datetime.strptime(user_feedback_deadline, '%Y-%m-%d').date()
    feedback_passed = is_deadline_passed(feedback_deadline_date)
    feedback_days_left = (feedback_deadline_date - date.today()).days
    
    deadline_color = "🔴" if feedback_passed else ("🟡" if feedback_days_left <= 3 else "🟢")
...
if (user_nomination_deadline != active_cycle['nomination_deadline'] or 
    user_feedback_deadline != active_cycle['feedback_deadline']):
    st.success("📝 **Note:** You have been granted deadline extensions. The dates above reflect your personalized deadlines.")
...
with col1:
    if st.button("📝 Request Feedback", type="primary", use_container_width=True):
        if nom_passed:
            st.error("The nomination deadline has passed. You can no longer request new feedback.")
        else:
            st.switch_page("app_pages/request_feedback.py")

with col2:
    pending_reviews = len(get_pending_reviews_for_user(user_id))
    button_text = f"✍️ Provide Feedback ({pending_reviews})" if pending_reviews > 0 else "✍️ Provide Feedback"
    
    if st.button(button_text, use_container_width=True):
        if feedback_passed:
            st.error("The feedback deadline has passed. You can no longer fill out feedback forms.")
        else:
    ...
    if pending_reviewer_requests:
        ...
        for req in pending_reviewer_requests[:3]:  # Show max 3
            ...
            with st.expander(f"Request from {requester_name} ({relationship})"):
                ...
                if st.button(f"Respond to {requester_name}", key=f"respond_{req['request_id']}"):
                    if nom_passed:
                        st.error("The nomination deadline has passed. All pending requests have been auto-accepted.")
                    else:
                        st.switch_page("app_pages/approve_nominations.py")
        
        if len(pending_reviewer_requests) > 3:
            st.info(f"... and {len(pending_reviewer_requests) - 3} more requests. Visit the Approve Nominations page to see all.")
    ...
    if pending_reviews:
        ...
        for review in pending_reviews[:3]:  # Show max 3
            ...
            with st.expander(f"Feedback for {requester_name} ({relationship})"):
                ...
                if st.button(f"Complete Feedback for {requester_name}", key=f"feedback_{review[0]}"):
                    if feedback_passed:
                        st.error("The feedback deadline has passed. You can no longer fill out feedback forms.")
                    else:
                        st.switch_page("app_pages/provide_feedback.py")
        ...
        if len(pending_reviews) > 3:
            st.info(f"... and {len(pending_reviews) - 3} more feedback forms. Visit the Provide Feedback page to see all.")
...
with st.expander("❓ Need Help?"):
    ...
    **Need more help?** Contact your HR team or manager.
    """)

# Footer
st.markdown("---")
st.caption(f"Review Cycle: {active_cycle['cycle_name']} | Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
