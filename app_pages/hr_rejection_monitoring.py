...
if not rejections:
    st.success("[Complete] No rejections found for the current cycle. The feedback process is running smoothly!")
else:
    st.warning(f"[Warning] Found {len(rejections)} rejection{'s' if len(rejections) > 1 else ''} to review.")

    # Tabs for different rejection types
    manager_rejections = [r for r in rejections if r['rejection_type'] == 'manager_rejection']
    reviewer_rejections = [r for r in rejections if r['rejection_type'] == 'reviewer_rejection']
        ...
        if manager_rejections:
            ...
            for rejection in manager_rejections:
                with st.expander(
                    ...
                    with cols[0]:
                        st.write(f"**Requester:** {rejection['requester_name']} ({rejection['requester_email']})")
                        st.write(f"**Rejected Reviewer:** {rejection['reviewer_name']}")
                        st.write(f"**Relationship:** {rejection['relationship_type'].replace('_', ' ').title()}")
        ...
        if reviewer_rejections:
            ...
            for rejection in reviewer_rejections:
                with st.expander(
                    ...
                    with cols[0]:
                        st.write(f"**Requester:** {rejection['requester_name']} ({rejection['requester_email']})")
                        st.write(f"**Reviewer Who Declined:** {rejection['reviewer_name']}")
                        st.write(f"**Relationship:** {rejection['relationship_type'].replace('_', ' ').title()}")
            ...
            st.success("[Complete] No reviewer rejections found.")
