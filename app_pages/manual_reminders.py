...
with tab1:
    ...
    with col1:
        recipient_type = st.selectbox(
            "Select Recipients",
            ["Individual Users", "All Users", "Users with Pending Tasks", "Custom Email List"]
        )
    ...
    if recipient_type == "Individual Users":
        ...
        task_type = st.selectbox(
            "Task Type",
            ["Pending Nominations", "Pending Reviews", "Pending Approvals"]
        )
        ...
        try:
            ...
            if active_cycle and task_type == "Pending Nominations":
                ...
                query = """
                    ...
                    AND fr.status != 'completed' AND u.is_active = 1
                """
                result = conn.execute(query, (active_cycle['cycle_id'],))
            ...
            if pending_users:
                st.info(f"Found {len(pending_users)} users with pending {task_type.lower()}")
            else:
    ...
    template = st.selectbox(
        "Choose Template",
        ["Custom", "Nomination Reminder", "Review Reminder", "General Deadline", "Final Warning"]
    )
    ...
    if template == "Nomination Reminder":
        ...
        <p>If you need assistance, please contact the HR team.</p>
        <p>Best regards,<br>The HR Team</p>
        """
    ...
    with col1:
        if st.button("📧 Send Reminders", type="primary", disabled=not (recipient_emails and subject and body)):
...
with tab2:
    ...
    active_cycle = get_active_review_cycle()
    if not active_cycle:
        st.warning("No active review cycle found. Deadline reminders are not available.")
    else:
        ...
        with col1:
            st.write("**Nomination Deadline:**", active_cycle.get('nomination_deadline', 'Not set'))
            st.write("**Feedback Deadline:**", active_cycle.get('feedback_deadline', 'Not set'))
        ...
        with col2:
            ...
            if active_cycle.get('nomination_deadline'):
                try:
                    nom_deadline = datetime.strptime(active_cycle['nomination_deadline'], '%Y-%m-%d').date()
        ...
        deadline_type = st.selectbox(
            "Reminder Type",
            ["Nomination Deadline", "Feedback Deadline"]
        )
        ...
        if st.button("🔔 Send Deadline Reminders"):
            ...
            try:
                ...
                deadline_date = active_cycle.get('nomination_deadline') if deadline_type == "Nomination Deadline" else active_cycle.get('feedback_deadline')
                ...
                for user in users:
                    if user['is_active']:
                        success = send_cycle_deadline_reminder(
                            ...
                            user_name=user['name'],
                            deadline_type=deadline_type.lower().replace(' deadline', ''),
...
with tab3:
    ...
    with col2:
        email_type_filter = st.selectbox(
            "Email Type", 
            ["All Types", "external_stakeholder_invite", "nominee_invite", "manager_approval_request", 
             "nomination_approved", "nomination_rejected", "feedback_submitted_notification", 
             "deadline_reminder", "manual_reminder", "password_reset"]
        )
        ...
        with st.spinner("Loading email log..."):
            try:
                ...
                if email_type_filter != "All Types":
                    log_entries = [entry for entry in log_entries if entry['email_type'] == email_type_filter]
                ...
                if success_filter == "Successful Only":
                    ...
                    log_entries = [entry for entry in log_entries if not entry['success']]
                ...
                try:
                    ...
                    stats_query = """
                        SELECT 
                            ...
                            COUNT(DISTINCT CASE WHEN fr.approval_status = 'approved' AND fr.status != 'completed' THEN fr.reviewer_id END) as pending_reviews,
                            COUNT(DISTINCT CASE WHEN fr.status = 'completed' THEN fr.request_id END) as completed_reviews,
...
# Footer
st.markdown("---")
st.caption("💡 **Tip:** Use deadline reminders strategically to maintain engagement without overwhelming users. Monitor the email log to ensure successful delivery.")
